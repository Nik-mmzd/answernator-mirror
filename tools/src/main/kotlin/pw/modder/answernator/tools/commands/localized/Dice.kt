...
class Dice: LocalizedCommand {
    ...
    override suspend fun action(bot: Bot, message: Message, texts: CommandLocaleBundle): CombinedMessageEmbed {
        ...
        val data = try {
            ...
            return texts.getErrorString("error.format").toMessage()
        } catch (e: DiceLimitExceededException) {
            return texts.formatString("error.limits", texts.getString("error.limits.${e.name}"), e.value, e.limit).toMessage()
        }
